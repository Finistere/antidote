...
from .constant import (Constant, ConstantFactory, ConstantValueConverter,
                       ConstantValueConverterMethod, ConstantValueProvider,
                       ConstantValueProviderFunction, ConstantValueProviderMethod,
...
class ConstantFactoryImpl(Generic[Arg, Value]):
    ...
    @classmethod
    def create(cls,
               provider: (ConstantValueProviderFunction[Arg, Value]
                          | ConstantValueProviderMethod[Arg, Value]),
    ...
    def __call__(self,
        ...
        type_ = (self.inferred_type_
                 if self.inferred_type_ is not None else
...
class TypedConstantFactoryImpl(Generic[Arg, ProvidedValue, Value]):
    ...
    converter: Optional[ConstantValueConverter[ProvidedValue]
                        | ConstantValueConverterMethod[ProvidedValue]]
    type_: Type[Value]
...
class ConstImpl(Singleton):
    ...
    def __init__(self) -> None:
        object.__setattr__(self, 'identity', ConstantFactoryImpl.create(
            provider=const_identity_provider
        ))
        object.__setattr__(self, 'env', ConstantFactoryImpl.create(
            provider=const_env_provider,
            converter=const_env_converter
        ))
    ...
    def provider(self,
        ...
        return ConstantProviderImpl[Arg, T](__func,
                                            const=ConstantFactoryImpl.create(provider=__func))
