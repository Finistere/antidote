...
#
# Notably faster lock when called from Cython at least.
#
# Original Author:
...
# Changes:
#   Changed formatting & cast pythread.PyThread_get_thread_ident() to long
...
cdef class FastRLock:
    ...
    cpdef bint acquire(self):
        return lock_lock(self, <long> pythread.PyThread_get_thread_ident())

    cpdef release(self):
        if self._owner != <long> pythread.PyThread_get_thread_ident():
            raise RuntimeError("cannot release un-acquired lock")
        unlock_lock(self)

cdef inline bint lock_lock(FastRLock lock, long current_thread) nogil:
    ...
    return _acquire_lock(lock, current_thread, pythread.WAIT_LOCK)
