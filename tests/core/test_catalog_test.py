...
from antidote import inject, new_catalog, PublicCatalog
...
def test_clone_override(
    ...
    original.add(x, factory=lambda _: Obj("x_value"))
    indirect_x = Obj()
    original.add(indirect_x, factory=lambda c: c[x], lifetime="transient")
    ...
    assert catalog[indirect_x] is x_value

    context = catalog.test.copy(frozen=frozen) if copy else catalog.test.clone(frozen=frozen)
    with context as overrides:
        ...
        # Proper copy of provider
        new_x_value = Obj()
        overrides[x] = new_x_value
        assert catalog[indirect_x] is new_x_value
...
def test_override_independent(catalog: PublicCatalog, test_context_of: TestContextOf) -> None:
    ...
    with test_context_of(catalog) as overrides:
        ...
        # properly removes the singletons
        assert catalog[x] is y
        ...
        # properly removes the tombstone
        assert catalog[x] is y
...
def test_nested_overrides(catalog: PublicCatalog) -> None:
    ...
    with catalog.test.copy() as overrides:
        ...
        with catalog.test.clone() as nested:
            ...
            with pytest.raises(RuntimeError, match="test context is not the latest one"):
                overrides[a] = b
...
def test_invalid_override() -> None:
    def f() -> Any:
        with new_catalog().test.empty() as x:
            return x

    overrides = f()
    gc.collect()

    with pytest.raises(RuntimeError, match="Invalid test context"):
        overrides["x"] = "x"
...
def test_debug(catalog: PublicCatalog, test_context_of: TestContextOf) -> None:
    with test_context_of(catalog) as overrides:
        ...
        assert (
            ...
            f"tests.core.test_catalog_test.test_debug.<locals>.create" in catalog.debug(y)
        )
...
def test_id(catalog: PublicCatalog, test_context_of: TestContextOf) -> None:
    ...
    with test_context_of(catalog):
        ...
        assert catalog.id.test_context_ids != current_id.test_context_ids
        assert catalog.private.id.name == current_private_id.name
        assert catalog.private.id.test_context_ids != current_private_id.test_context_ids
